...
preference_table = {
    "default_show_half_glider": (bool, False),
...
def mesh_sep(mesh, color, draw_lines=False, soft_edges=True):
    ...
    if draw_lines:
        ...
        line_mat.diffuseColor = (0.0, 0.0, 0.0)
...
class OGGlider(OGBaseObject):
    def __init__(self, obj, parametric_glider=None, import_path=None):
        obj.addProperty(
            "App::PropertyPythonObject", "GliderInstance", "object", "GliderInstance", 2
        )
        obj.addProperty(
        ...
        obj.addProperty("App::PropertyLinkList", "airfoils", "object", "airfoils", 0)
    ...
    def execute(self, obj):
        if hasattr(obj, "airfoils") and len(obj.airfoils) > 0:
            airfs = [
                Profile2D(i.Proxy.get_airfoil(i).coordinates) for i in obj.airfoils
            ]
...
class OGGliderVP(OGBaseVP):
    def __init__(self, view_obj):
        view_obj.addProperty("App::PropertyBool", "ribs", "visuals", "show ribs")
        ...
        view_obj.addProperty(
            "App::PropertyInteger", "hole_num", "accuracy", "number of hole vertices"
        )
        view_obj.addProperty("App::PropertyInteger", "line_num", "accuracy", "line_num")
        view_obj.addProperty("App::PropertyEnumeration", "hull", "visuals")
        view_obj.addProperty(
            "App::PropertyBool", "half_glider", "visuals", "show only one half"
        )
        view_obj.addProperty(
            "App::PropertyBool", "fill_ribs", "visuals", "fill ribs with polygons"
        )
        view_obj.addProperty("App::PropertyFloat", "x", "postion", "set x-position")
        view_obj.addProperty("App::PropertyFloat", "y", "postion", "set y-position")
        view_obj.addProperty("App::PropertyFloat", "z", "postion", "set z-position")
    ...
    def update_glider(
        ...
        draw_glider(
            self.glider,
            vis_glider=self.vis_glider,
            midribs=midribs,
            hole_num=hole_num,
            profile_num=profile_numpoints,
            hull=hull,
            ribs=ribs,
            fill_ribs=fill_ribs,
        )
...
def draw_glider(
    ...
    if (
        vis_glider.getByName("hull") is None
    ):  # TODO: fix bool(sep_without_children) -> False pivy
    ...
    if ribs and draw_ribs:
        ...
        if msh.vertices is not None:
            rib_sep += [mesh_sep(msh, (0.3, 0.3, 0.3))]
        if line_msh.vertices is not None:
            rib_sep += [mesh_sep(line_msh, (0.3, 0.3, 0.3), draw_lines=True)]
        vis_glider += [rib_sep]
