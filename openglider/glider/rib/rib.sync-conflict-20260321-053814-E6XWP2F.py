...
class SingleSkinRib(Rib):
    ...
    def align_all(self, data):
        """Align 2D→3D, then warp intrados/bow points to follow gravity.

        In a single-skin zone, there is no intrados fabric to constrain
        the rib's lower portion. The bow/attachment points hang under
        gravity/line pull, so they shift laterally relative to the rib
        plane (which is tilted by arcang).

        The warp is computed directly from arcang — no lineset needed,
        and automatically handles mirrored ribs (arcang negates).
        """
        result = super().align_all(data)

        # No warp needed if rib is vertical (arcang ≈ 0)
        if abs(self.arcang) < 1e-4:
            return result
        ...
        # Warp direction: rib plane normal (perpendicular to profile plane)
        # Computed on-the-fly from rotation_matrix → correct for mirrored ribs
        rib_normal = np.array(self.rotation_matrix([0, 0, 1]))

        # Warp magnitude from arc angle: tan(arcang) gives the lateral
        # shift per unit of depth needed to align with vertical pull
        warp_factor = np.tan(self.arcang)

        # Shift all points below the chord (y < 0 = bows/attachment zone)
        # proportionally to their depth. At y=0 the shift is zero (smooth
        # transition from extrados). Deeper bow points shift more.
        for i, p2d in enumerate(data_2d):
            y_val = p2d[1] if len(p2d) > 1 else 0
            if y_val < 0:  # below chord = bow/intrados
                depth = abs(y_val) * self.chord
                result[i] = result[i] + rib_normal * (depth * warp_factor)
