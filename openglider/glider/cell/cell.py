...
class Cell(CachedObject):
    ...
    def rename_panels(self, seperate_upper_lower=False):
        if seperate_upper_lower:
            ...
            for panel_no, panel in enumerate(upper):
                panel.name = self.panel_naming_scheme_upper.format(
                    cell=self, panel_no=panel_no + 1
                )
            for panel_no, panel in enumerate(lower):
                panel.name = self.panel_naming_scheme_lower.format(
                    cell=self, panel_no=panel_no + 1
                )
            ...
            for panel_no, panel in enumerate(self.panels):
                panel.name = self.panel_naming_scheme.format(
                    cell=self, panel=panel, panel_no=panel_no + 1
                )

    def rename_parts(self, seperate_upper_lower=False):
        for diagonal_no, diagonal in enumerate(self.diagonals):
            diagonal.name = self.diagonal_naming_scheme.format(
                cell=self, diagonal=diagonal, diagonal_no=diagonal_no + 1
            )

        for strap_no, strap in enumerate(self.straps):
            strap.name = self.strap_naming_scheme.format(
                cell=self, strap=strap, strap_no=strap_no + 1
            )

        for minirib_no, minirib in enumerate(self.miniribs):
            minirib.name = self.minirib_naming_scheme.format(
                cell=self, minirib=minirib, minirib_no=minirib_no + 1
            )
    ...
    def basic_cell(self) -> BasicCell:
        return BasicCell(
            self.rib1.profile_3d, self.rib2.profile_3d, self.ballooning_phi
        )
    ...
    def _child_cells(self):
        ...
        """
        cells = []
        for leftrib, rightrib in zip(
            self.rib_profiles_3d[:-1], self.rib_profiles_3d[1:]
        ):
    ...
    def get_spline(self, numribs, u_poles=20, v_poles=4, u_degree=3, v_degree=3):
        ...
        u_knots = nurbs.create_knots_vector(
            u_min=0, u_max=1, degree=u_degree, num_poles=u_poles
        )
    ...
    @cached_property(
        "ballooning", "rib1.profile_2d.numpoints", "rib2.profile_2d.numpoints"
    )
    def ballooning_phi(self):
        x_values = self.rib1.profile_2d.x_values
        balloon = [self.ballooning[i] for i in x_values]
        return HashedList(
            [Ballooning.arcsinc(1.0 / (1 + bal)) if bal > 0 else 0 for bal in balloon]
        )
    ...
    def area(self):
        ...
        return 0.5 * (
            norm(np.cross(p1_2 - p1_1, p2_1 - p1_1))
    ...
    def mean_rib(self, num_midribs=8) -> Profile2D:
        ...
        return mean_rib * (1.0 / num_midribs)
    ...
    def get_mesh(self, numribs=0, with_numpy=False, half_cell=False):
        ...
        grid = self.get_mesh_grid(
            numribs=numribs, with_numpy=with_numpy, half_cell=half_cell
        )
        ...
        for rib_left, rib_right in zip(grid[:-1], grid[1:]):
            ...
            for i in range(numpoints):
                ...
                pol = Polygon(
                    [rib_left[i], rib_right[i], rib_right[i_next], rib_left[i_next]]
                )
        ...
        mesh = Mesh(
            {"hull": quads},
            {
                self.rib1.name: grid[0],
                self.rib2.name: grid[-1],
                "trailing_edge": trailing_edge,
            },
        )
        return mesh

    def get_mesh_mapping_grid(
        self, cell_number=0, numribs=0, with_numpy=False, half_cell=False
    ):
        """
        Get Cell-grid
        ...
        for rib_no in rib_indices:
            y = rib_no / max(numribs, 1)
            rib = (
                self.rib1.profile_2d.get_data(negative_x=True) * (1 - y)
                + self.rib2.profile_2d.get_data(negative_x=True) * y
            )
            y += cell_number
            rib = np.array(
                [rib.T[0], np.array([y] * len(rib)), rib.T[1]]
            ).T  # insert y-value
            grid.append(Vertex.from_vertices_list(rib))
        return grid

    def get_mesh_mapping(
        self, cell_number=0, numribs=0, with_numpy=False, half_cell=False
    ):
        """
        Get Cell-mesh
        ...
        grid = self.get_mesh_mapping_grid(
        ...
        for rib_left, rib_right in zip(grid[:-1], grid[1:]):
            ...
            for i in range(numpoints - 1):
                ...
                quads.append(pol)
        mesh = Mesh(
            {"hull": quads}, {self.rib1.name: grid[0], self.rib2.name: grid[-1]}
        )
        return mesh
    ...
    def get_flattened_cell(self, numribs=50, y_start=0.0, y_end=1.0):
        ...
        ballooned = [PolyLine2D(left_bal), PolyLine2D(right_bal)]
        ...
        return {"inner": inner, "ballooned": ballooned}
