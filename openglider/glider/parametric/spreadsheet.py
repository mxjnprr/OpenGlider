...
class GliderSpreadsheet:
    ...
    def set_geometry(self):
        ...
        arc_angles_rad = self.glider2d.arc.get_cell_angles(
            self.glider2d.shape.rib_x_values
        ) + [0]
    ...
    def set_rib_parts(self):
        ...
        # attachment points
        per_rib = [
            self.glider2d.lineset.get_upper_nodes(rib_no)
            for rib_no in range(self.glider2d.shape.half_rib_num)
        ]
    ...
    def set_parametric(self):
        ...
        table.add_columns(
            *self._transpose_columns(self.glider2d.shape.rib_distribution, "Dist")
        )
        table.add_columns(*self._transpose_columns(self.glider2d.aoa, "AOA"))
        table.add_columns(
            *self._transpose_columns(self.glider2d.profile_merge_curve, "Profile_merge")
        )
        table.add_columns(
            *self._transpose_columns(
